...
async function login(username, password){
  try{
    ...
    if(saltResult.error){
      showMessage("loginMessage", "Database belum siap: fungsi login belum terbaca. Jalankan file supabase-reset-full.sql di SQL Editor, lalu reload website.");
    ...
    if(result.error){
      showMessage("loginMessage", "Login gagal dari database. Jalankan supabase-reset-full.sql, lalu coba lagi.");
    ...
    if(!row){
      showMessage("loginMessage", "Username atau password salah. Untuk admin gunakan username: admin dan password: Admin123.");
      return;
    }
...
function getMbtiCharacterSuggestion(type){
  const suggestions = {
    ISTJ: "Pertahankan kedisiplinan dan ketelitian Anda, tetapi beri ruang untuk fleksibilitas ketika situasi berubah. Kekuatan utama ISTJ akan semakin menonjol ketika struktur kerja diimbangi dengan komunikasi yang hangat.",
    ISFJ: "Gunakan kepedulian dan loyalitas Anda sebagai kekuatan, tetapi jangan lupa menetapkan batas pribadi. ISFJ akan berkembang lebih sehat ketika mampu membantu orang lain tanpa mengabaikan kebutuhan diri sendiri.",
    INFJ: "Percayai intuisi dan nilai ideal Anda, namun tetap pecah tujuan besar menjadi langkah kecil yang realistis. INFJ akan semakin kuat ketika visi jangka panjang diimbangi dengan batas energi yang jelas.",
    INTJ: "Manfaatkan kemampuan strategi dan analisis Anda untuk menyusun rencana matang, tetapi latih komunikasi agar ide lebih mudah diterima orang lain. INTJ akan lebih efektif ketika logika dipadukan dengan empati.",
    ISTP: "Gunakan kemampuan praktis dan ketenangan Anda untuk menyelesaikan masalah nyata, tetapi biasakan menyampaikan rencana kepada orang sekitar. ISTP akan berkembang ketika kebebasan berpikir tetap diiringi komitmen yang jelas.",
    ISFP: "Jaga keaslian diri dan kepekaan estetis Anda, tetapi latih keberanian untuk menyampaikan pendapat. ISFP akan lebih percaya diri ketika kreativitas diimbangi dengan struktur kerja yang sederhana.",
    INFP: "Rawat idealisme dan empati Anda, tetapi pilih prioritas agar energi tidak terlalu tersebar. INFP akan tumbuh ketika nilai pribadi diterjemahkan menjadi tindakan konkret dan konsisten.",
    INTP: "Gunakan rasa ingin tahu dan kemampuan analitis Anda untuk memahami masalah secara mendalam, tetapi jangan berhenti hanya pada ide. INTP akan lebih kuat ketika pemikiran konseptual diikuti eksekusi bertahap.",
    ESTP: "Manfaatkan keberanian dan respons cepat Anda dalam situasi dinamis, tetapi pikirkan dampak jangka panjang sebelum mengambil keputusan. ESTP akan semakin matang ketika spontanitas diimbangi perencanaan.",
    ESFP: "Pertahankan energi sosial dan kemampuan menciptakan suasana positif, tetapi jaga fokus pada tujuan yang sedang dikerjakan. ESFP akan berkembang ketika antusiasme dipadukan dengan disiplin prioritas.",
    ENFP: "Gunakan kreativitas dan kemampuan melihat peluang sebagai kekuatan utama, tetapi batasi distraksi dari terlalu banyak ide baru. ENFP akan semakin efektif ketika inspirasi diikuti konsistensi.",
    ENTP: "Pertahankan keberanian mengeksplorasi ide dan menantang asumsi, tetapi pastikan diskusi tetap membangun. ENTP akan lebih berdampak ketika kecerdasan argumentatif disertai penyelesaian nyata.",
    ESTJ: "Gunakan ketegasan dan kemampuan mengatur sistem untuk mencapai target, tetapi tetap beri ruang bagi gaya kerja orang lain. ESTJ akan semakin kuat ketika kepemimpinan tegas diimbangi kemampuan mendengar.",
    ESFJ: "Pertahankan kehangatan dan kemampuan menjaga hubungan, tetapi belajar mengatakan tidak ketika diperlukan. ESFJ akan berkembang ketika kepedulian sosial tetap disertai batas diri yang sehat.",
    ENFJ: "Gunakan kemampuan memotivasi dan merangkul orang lain untuk membangun tujuan bersama, tetapi jangan memikul semua tanggung jawab sendiri. ENFJ akan lebih seimbang ketika empati diiringi batas personal.",
    ENTJ: "Manfaatkan visi, keberanian mengambil keputusan, dan orientasi target Anda, tetapi beri ruang pada proses orang lain. ENTJ akan lebih efektif ketika ambisi strategis dipadukan dengan komunikasi yang suportif."
  };
  return suggestions[type] || "Kenali kekuatan utama Anda, lalu gunakan hasil ini sebagai bahan refleksi untuk mengembangkan kebiasaan, komunikasi, dan cara mengambil keputusan yang lebih seimbang.";
}

function renderResult(type, scores){
  const d = typeData[type];
  const participantName = currentSession?.full_name || currentSession?.username || "-";
  ...
  $("resultContent").innerHTML = `
    <div class="print-watermark print-only">checkyourmbti</div>
    <div class="print-header print-only">
      <div class="print-kicker">Laporan Hasil Tes Kepribadian</div>
      <h3>checkyourmbti</h3>
      <div class="print-meta">
        <div class="print-meta-item"><span>Nama Peserta</span><strong>${participantName}</strong></div>
        <div class="print-meta-item"><span>Waktu Cetak</span><strong>${printedAt}</strong></div>
      </div>
    </div>

    <div class="participant-summary info-card">
      <h3>Informasi Peserta</h3>
      <div class="participant-summary-grid">
        <div><span>Nama Peserta</span><strong>${participantName}</strong></div>
        <div><span>Tipe MBTI</span><strong>${type} - ${d.name}</strong></div>
      </div>
    </div>
    ...
    <div class="info-card character-advice">
      <h3>Masukan Karakter MBTI</h3>
      <p>${getMbtiCharacterSuggestion(type)}</p>
    </div>
...
function validateConfig(){
  if(!window.supabase){
    showMessage("loginMessage", "Library Supabase gagal dimuat. Pastikan internet aktif dan script Supabase tidak diblokir browser.");
    return false;
  }
  if(!SUPABASE_URL.includes("supabase.co") || SUPABASE_ANON_KEY.startsWith("GANTI_")){
    showMessage("loginMessage", "Konfigurasi Supabase belum benar. Pastikan config.js sudah berisi SUPABASE_URL dan SUPABASE_ANON_KEY.");
    return false;
  }
  if(!db){
    db = window.supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  $("downloadPrintBtn").addEventListener("click", downloadResult);
