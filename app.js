...
async function login(username, password){
  ...
  const saltResult = await db.rpc("mbti_get_salt", { p_username: username.trim().toLowerCase() });
  if(saltResult.error || !saltResult.data){ showMessage("loginMessage", "Username atau password salah."); return; }
  const hash = await hashPassword(password, saltResult.data);
  const result = await db.rpc("mbti_login", { p_username: username.trim().toLowerCase(), p_password_hash: hash });
  if(result.error){ showMessage("loginMessage", result.error.message || "Login gagal."); return; }
  const row = Array.isArray(result.data) ? result.data[0] : null;
  if(!row){ showMessage("loginMessage", "Username atau password salah."); return; }
  saveSession(row);
