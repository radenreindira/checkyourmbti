-- MBTI SUPABASE FULL RESET
-- Gunakan file ini jika login masih gagal atau fungsi database belum terbaca.
-- PERINGATAN: File ini menghapus ulang tabel MBTI dan data peserta/hasil lama.
-- Jika belum ada data penting, aman dijalankan.
--
-- Setelah sukses, login admin:
-- Username: admin
-- Password: Admin123

drop function if exists public.mbti_get_salt(text);
drop function if exists public.mbti_login(text, text);
drop function if exists public.mbti_logout(text);
drop function if exists public.mbti_is_admin(text);
drop function if exists public.mbti_create_participant(text, text, text, text, text);
drop function if exists public.mbti_list_participants(text);
drop function if exists public.mbti_reset_participant_password(text, uuid, text, text);
drop function if exists public.mbti_delete_participant(text, uuid);
drop function if exists public.mbti_save_result(text, text, jsonb, jsonb);

drop table if exists public.mbti_results cascade;
drop table if exists public.mbti_sessions cascade;
drop table if exists public.mbti_accounts cascade;

create extension if not exists pgcrypto;

create table public.mbti_accounts (
...
create table public.mbti_sessions (
